...
class WebSiteHandler:
    ...
    def get_urls_list(self, page_from: int = 1, page_to: int = 1) -> list[str]:
        url_list = self.parser.get_all_last_flats_links(page_from=page_from, page_to=page_to)
        return url_list

    def build_flats_list(self, urls_list: list[str]) -> list[Flat]:
        obj_list = self.parser.enrich_links_to_flats(urls_list)
        return obj_list

    def save_to_db_with_staticstorage(self, obj_list: list, sql_db: SQLDataBase, static_db: DataBase) -> None:
    ...
    def save_to_db(self, obj_list: list[Flat]) -> None:
    ...
    def run(self, page_from: int = 1, page_to: int = 1) -> None:
